...
public ResponseEntity<?> listarPorAluno(@PathVariable Long idAluno) {
    ...
    List<MatriculaDTO> resposta = matriculas.stream().map(m -> 
        new MatriculaDTO(
    ...
    public ResponseEntity<List<DisciplinaResponseDTO>> listarPorProfessor(@PathVariable Long idProfessor) {
        return ResponseEntity.ok(disciplinaService.listarPorProfessor(idProfessor));
    }
